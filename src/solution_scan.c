	...
	while (q <= 9)
		{
			if(s[i][j][q] == '.' && (s[i][j][0] == '.' || s[i][j][0] == '+'))
				{
					s[i][j][0] = q + '0';
				    solution_check_lines(s, i, j);
				    solution_check_squares(s, i, j);
					s[i][j][0] = '.';
				}
			++q;
		}
...
{
		if (0 <= i && i < 3 && 0 <= j && j < 3)
			solution_check_top_left(s, i, j);
		else if (0 <= i && i < 3 && 3 <= j && j < 6)
			solution_check_top_mid(s, i, j);
		else if (0 <= i && i < 3 && 6 <= j && j < 9)
			solution_check_top_right(s, i, j);
		else if (3 <= i && i < 6 && 0 <= j && j < 3)
			solution_check_mid_left(s, i, j);
		else if (3 <= i && i < 6 && 3 <= j && j < 6)
			solution_check_mid_mid(s, i, j);
		else if (3 <= i && i < 6 && 6 <= j && j < 9)
			solution_check_mid_right(s, i, j);
		else if (6 <= i && i < 9 && 0 <= j && j < 3)
			solution_check_bot_left(s, i, j);
		else if(6 <= i && i < 9 && 3 <= j && j < 6)
			solution_check_bot_mid(s, i, j);
		else if(6 <= i && i < 9 && 6 <= j && j < 9)
			solution_check_bot_right(s, i, j);
	return ;
}
